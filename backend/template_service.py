...
def _inserer_reference_client(docx_path: Path, ref) -> Path:
    ...
    try:
        ...
        def make_ref_el(style_ref=None):
            ...
            Remontée de ~2 lignes SANS toucher au flux : on décale UNIQUEMENT le
            texte de ce paragraphe vers le haut via `w:position` (positionnement
            ciblé, 24 pt) et on fige la hauteur de ligne à sa valeur naturelle
            (`w:spacing lineRule="exact" line="370"`, soit 18,5 pt) afin que le
            cadre de ligne ne grandisse pas. Le paragraphe conserve donc EXACTEMENT
            sa hauteur dans le flux → « Proposition », l'adresse chantier, le pied
            de page et la page suivante ne bougent pas. Aucun paragraphe vide, aucun
            changement de marge ni de hauteur de section."""
            from docx.oxml import OxmlElement
            from docx.oxml.ns import qn as _qn
            p = OxmlElement("w:p")
            pPr = OxmlElement("w:pPr")
            sp = OxmlElement("w:spacing")            # hauteur de ligne figée = naturelle
            sp.set(_qn("w:line"), "370"); sp.set(_qn("w:lineRule"), "exact")
            pPr.append(sp)
            ...
            pos = OxmlElement("w:position"); pos.set(_qn("w:val"), "48")           # texte remonté 24 pt (≈2 lignes)
            rPr.append(pos)
            r.append(rPr)
            t = OxmlElement("w:t")
...
def generer_devis(template_path: Path, data: Dict, output_path: Path) -> Path:
    ...
    if template_stem in COPRO_PETITE_TEMPLATE_CODES:
        ...
        # Marge haute : le 1er contenu de CHAQUE page à saut (Poste 2, pages fixes,
        # CGV) démarre sous le logo d'en-tête. Appliqué APRÈS le nettoyage des vides
...
def _generer_copro_petite_preserve_layout(template_path: Path, data: Dict, output_path: Path) -> Path:
    ...
    def insert_postes_remise_etat():
        ...
        if fixed3 is not None:
            ...
            # keep_next=False sur l'intro : on colle le titre à son intro (via le
            # titre), mais on NE colle PAS l'intro à la 1re zone. Sinon, quand cette
            # zone (table haute, indivisible) ne tient pas après la marge haute,
            # toute la chaîne bascule à la page suivante et laisse la marge haute
            # seule sur une page blanche parasite. La 1re zone peut ainsi couler
            # librement pendant que titre + intro restent sous le logo.
            emit(intro, body_ref, after=10, keep_next=False)
...
def _marge_haute_sous_logo(docx_path: Path) -> Path:
    ...
    def _a_saut(el):
        return el.tag == _qn("w:p") and any(
            br.get(_qn("w:type")) == "page" for br in el.findall(".//" + _qn("w:br")))
    ...
    for p, marge_pt in cibles:
        ...
        prev = el.getprevious()
        while prev is not None and _est_separateur(prev):
            rem = prev
            prev = prev.getprevious()
            rem.getparent().remove(rem)
